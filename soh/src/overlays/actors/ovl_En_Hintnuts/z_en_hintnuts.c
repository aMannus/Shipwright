...
void EnHintnuts_SetupLeave(EnHintnuts* this, PlayState* play) {
    ...
    this->actor.flags |= ACTOR_FLAG_4;
    Audio_PlayActorSound2(&this->actor, NA_SE_EN_NUTS_DAMAGE);
    OTRActor_Spawn(&play->actorCtx, play, ACTOR_EN_ITEM00, this->actor.world.pos.x, this->actor.world.pos.y,
...
void EnHintnuts_ThrowNut(EnHintnuts* this, PlayState* play) {
    ...
    if (this->actor.xzDistToPlayer < 120.0f) {
        ...
        if (OTRActor_Spawn(&play->actorCtx, play, ACTOR_EN_NUTSBALL, nutPos.x, nutPos.y, nutPos.z,
