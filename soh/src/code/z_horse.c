...
void func_8006D0EC(PlayState* play, Player* player) {
    ...
    if ((AREG(6) != 0) && (Flags_GetEventChkInf(0x18) || (DREG(1) != 0))) {
        player->rideActor = OTRActor_Spawn(&play->actorCtx, play, ACTOR_EN_HORSE, player->actor.world.pos.x,
        ...
        horseActor =
            OTRActor_Spawn(&play->actorCtx, play, ACTOR_EN_HORSE, 3586.0f, 1413.0f, -402.0f, 0, 0x4000, 0, 1);
        horseActor->room = -1;
    } else if ((gSaveContext.entranceIndex == 1230) && (gSaveContext.eventChkInf[1] & 0x100)) {
        Actor* horseActor =
            OTRActor_Spawn(&play->actorCtx, play, ACTOR_EN_HORSE, -25.0f, 0.0f, -1600.0f, 0, -0x4000, 0, 1);
        ...
        if (func_8006CFC0(gSaveContext.horseData.scene)) {
            Actor* horseActor = OTRActor_Spawn(&play->actorCtx, play, ACTOR_EN_HORSE,
        ...
        Actor* horseActor =
            OTRActor_Spawn(&play->actorCtx, play, ACTOR_EN_HORSE, 0.0f, 0.0f, -500.0f, 0, 0, 0, 1);
        ASSERT(horseActor != NULL);
        ...
        for (i = 0; i < ARRAY_COUNT(horseSpawns); i++) {
            ...
            if (horseSpawn->scene == play->sceneNum) {
                Actor* horseActor =
                    OTRActor_Spawn(&play->actorCtx, play, ACTOR_EN_HORSE, horseSpawn->pos.x, horseSpawn->pos.y,
        ...
        if ((DREG(1) == 0) && (play->sceneNum == SCENE_SOUKO) && !IS_DAY) {
            OTRActor_Spawn(&play->actorCtx, play, ACTOR_EN_HORSE, 0.0f, 0.0f, -60.0f, 0, 0x7360, 0, 1);
        }
    }
}
...
void func_8006D684(PlayState* play, Player* player) {
    ...
    if ((gSaveContext.entranceIndex == 0x028A || gSaveContext.entranceIndex == 0x028E ||
        ...
        player->rideActor = OTRActor_Spawn(&play->actorCtx, play, ACTOR_EN_HORSE, spawnPos.x, spawnPos.y,
        ...
        player->rideActor =
            OTRActor_Spawn(&play->actorCtx, play, ACTOR_EN_HORSE, 894.0f, 0.0f, -2084.0f, 0, -0x7FFF, 0, 5);
        ASSERT(player->rideActor != NULL);
        ...
        for (i = 0; i < ARRAY_COUNT(D_8011F9B8); i++) {
            if ((play->sceneNum == D_8011F9B8[i].scene) &&
                ...
                if (D_8011F9B8[i].type == 7) {
                    ...
                    player->rideActor = OTRActor_Spawn(&play->actorCtx, play, ACTOR_EN_HORSE,
                    ...
                    OTRActor_Spawn(&play->actorCtx, play, ACTOR_EN_HORSE, D_8011F9B8[i].pos.x,
