...
void Patched_InterpFrameTable(s32 limbCount, Vec3s* dst, Vec3s* start, Vec3s* target, f32 weight) {
    if (weight >= 1.0f) {
    ...
    for (s32 i = 0; i < limbCount; i++, dst++, start++, target++) {
        ...
        if (dx <= 0xC000 || dx >= 0x4000)
            ++numLargeRot;
        if (dy <= 0xC000 || dy >= 0x4000)
            ++numLargeRot;
        if (dz <= 0xC000 || dz >= 0x4000)
            ++numLargeRot;
